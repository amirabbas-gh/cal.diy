...
export const revalidateSettingsProfile = createServerFn().handler(async () => {
  queryClient.invalidateQueries({ queryKey: ['next-cache', 'path', "/settings/my-account/profile"] });
});

export const revalidateSettingsCalendars = createServerFn().handler(async () => {
  queryClient.invalidateQueries({ queryKey: ['next-cache', 'path', "/settings/my-account/calendars"] });
});
