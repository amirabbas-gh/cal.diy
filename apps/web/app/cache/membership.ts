...
export const getCachedHasTeamPlanQueryOptions = {
  queryKey: ['next-cache', "getCachedHasTeamPlan"] as const,
  queryFn: getCachedHasTeamPlan,
};

export const revalidateHasTeamPlan = createServerFn().handler(async () => {
  queryClient.invalidateQueries({ queryKey: ['next-cache', 'tag', CACHE_TAGS.HAS_TEAM_PLAN] });
});
