...
export async function parseUrlFormData(req: Request): Promise<Record<string, any>> {
  try {
    ...
    log.error(`Invalid Url Form Data: ${e} from path ${new URL(req.url)}`);
...
export async function parseMultiFormData(req: Request): Promise<Record<string, any>> {
  try {
    ...
    log.error(`Invalid Multi Form Data: ${e} from path ${new URL(req.url)}`);
...
export async function parseRequestData(req: Request): Promise<Record<string, any>> {
  ...
  if (contentType.includes("application/json")) {
    try {
      ...
      log.error(`Invalid JSON: ${e} from path ${new URL(req.url)}`);
  ...
  log.error(`Unsupported content type: ${contentType} from path ${new URL(req.url)}`);
