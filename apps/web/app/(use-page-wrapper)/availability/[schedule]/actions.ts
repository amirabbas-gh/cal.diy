...
export const revalidateSchedulePage = createServerFn().handler(async (scheduleId: number) => {
  queryClient.invalidateQueries({ queryKey: ['next-cache', 'path', `/availability/${scheduleId}`] });
});
