...
function Bookings(props: { calUsername: string; calEmail: string }) {
  const params = Route.useParams();
  const router = useNavigate();

  const { isLoading, data: booking, refetch } = useBooking((params.bookingUid as string) ?? "");
                ...
                <div className="mx-2 my-3 text-center">
                  <p data-testid="booking-redirect-or-cancel-links">
                    ...
                    <button
                      className="underline"
                      onClick={() => {
                        router({ to: `/booking?rescheduleUid=${booking?.uid}&eventTypeSlug=${booking?.eventType.slug}&rescheduledBy=${props.calEmail}` });
                      }}>
                      Reschedule
                    </button>{" "}
                    or{" "}
...
export const Route = createFileRoute("/$bookingUid")({
  component: Bookings,
});
